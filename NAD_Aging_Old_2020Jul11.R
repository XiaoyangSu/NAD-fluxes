...
NADNAMmod0D1 <- function(Time, State, Pars) {
  with(as.list(c(State, Pars)), {
    ####NAM tracer####
    dNAM_S0.NAM <- 0.6971^Time*log(0.6971)
    dNAM_S3.NAM <- 0
    dNAM_S4.NAM <- -0.6971^Time*log(0.6971)
    dTrp_0.NAM <- 0
    dTrp_3.NAM <- 0
    dTrp_4.NAM <- 0
...
NADNAMmod0D2 <- function(Time, State, Pars) {
  with(as.list(c(State, Pars)), {
    ####NAM tracer####
    dNAM_S0.NAM <- -0.0798/Time
    dNAM_S3.NAM <- 0.0483/Time
    dNAM_S4.NAM <- 0.0315/Time
    dTrp_0.NAM <- 0
    dTrp_3.NAM <- 0
    dTrp_4.NAM <- 0
    dNAD_0.NAM <- (f1*(Trp_0.NAM-NAD_0.NAM)+
...
NADNAMmod0D3 <- function(Time, State, Pars) {
  with(as.list(c(State, Pars)), {
    ...
    ####Tryptophan tracer#### 
    dNAM_S0.Trp <- -0.0582/Time
    dNAM_S6.Trp <- 0.0582/Time
    dTrp_0.Trp <- 0
    dTrp_6.Trp <- 0
    dNAD_0.Trp <- (f1*(Trp_0.Trp-NAD_0.Trp)+
                     f3*(NAM_0.Trp-NAD_0.Trp))/cNAD
...
yini0 <- c(NAM_S0.NAM=1,NAM_S3.NAM=0,NAM_S4.NAM=0,Trp_0.NAM=1,Trp_3.NAM=0,Trp_4.NAM=0,
           ...
           NAM_S0.Trp=1,NAM_S6.Trp=0,Trp_0.Trp=0.40,Trp_6.Trp=0.60,
...
InputData <- read.csv("E:/XSu/Papers/NAD and Aging/2020May/Jul11/Input_SDAvg_Old_0711.csv",header = FALSE,stringsAsFactors = FALSE)
...
system.time(for (TissueCounter in 1:12) {
  ...
  write.csv(NADNAM2(Fluxes.Results),file=paste(colnames(FluxMatrix)[TissueCounter],"_Aged_SDAvg_TimeCourse.csv",sep=""))
  write.csv(FluxMatrix,file="Output_Aged_SDAvg_2020Jul11.csv")
})
